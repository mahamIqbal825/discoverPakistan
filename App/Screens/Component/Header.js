...
class Header extends React.Component {
    ...
    render(){
        ...
        const isRenderPlayer = this.props.isRenderPlayer !== undefined ? this.props.isRenderPlayer : true
        ...
        return (<View style={styles.containerStyle}>
                <Row style={styles.headRow}>
                    ...
                    <Row style={styles.profileRow}>
                        <Text ellipsizeMode="tail" numberOfLines={1} style={styles.profileName}>{isNullRetNull(userData.name, "")}</Text>
                        <TouchableOpacity
                            ...
                            }}>
                            {
                                this.props.isLoggedIn ?
                                    <Image source={{ uri:isNullRetNull(userData.img, 'https://') }} style={styles.profileImg} />
                ...
                { isShowSearch && <SearchBox { ...this.props } tempArray={tempArray} filterFun={filterFun} /> }
                { isRenderPlayer &&
                    <Row style={styles.webViewRow}>
                        { renderPlayer && renderPlayer() }
                    </Row>
                }

                { isShowSubHeader && <SubHeader navigation={navigation} toggleDrawer={toggleDrawer} /> }
        </View>);
    }
};
